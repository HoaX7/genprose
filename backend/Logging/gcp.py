...
class GCPLogger(LogAbstract):
    ...
    def info(self, *args, **kwargs):
        """
            Remove Dict from args.
            Need to improve this logic since there's a possibility
            of more than 1 dict in the tuple.
        """
        log_str = args[:len(args) - 1]
        log_str = " ".join(log_str)

        """
            Labels can be used in 'extra' to set a context
        """
        logger.info(log_str, extra={"json_fields": kwargs})

    def warn(self, *args, **kwargs):
        log_str = args[:len(args) - 1]
        log_str = " ".join(log_str)
        logger.warn(log_str, extra={"json_fields": kwargs})

    def error(self, *args, **kwargs):
        log_str = args[:len(args) - 1]
        log_str = " ".join(log_str)
        logger.error(log_str, extra={"json_fields": kwargs})
